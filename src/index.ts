import {Telegram} from 'telegram-js'
import * as MTProto from '@goodmind/telegram-mt-node'
import * as TypeLanguage from '@goodmind/telegram-tl-node'
import {restoreAuthKey} from './helpers'
import {enable} from 'debug';

import * as schema from './api-tlschema-57.json'

enable('*');

const APP_ID = 49631;
const APP_HASH = 'fb050b8f6771e15bfda5df2409931569';
const SERVER = {
  host: "149.154.167.50",
  port: "443"
};

const telegram = new Telegram(MTProto, TypeLanguage);
telegram.useSchema(schema);
addPublicKeys(telegram);

const config = {
  id: APP_ID,
  hash: APP_HASH,
  version: '1.0.1',
  langCode: 'en',
  deviceModel: navigator.vendor,
  systemVersion: navigator.userAgent,
  authKey: restoreAuthKey(),
};

// on Node.js side, is better to use TcpConnection instead
let connection = new MTProto.net.HttpConnection(SERVER);

// You can create as many clients as necessary. Usually, you will have two
// instances, one for API calls and one for file transport
let client = telegram.createClient();
client.setConnection(connection);

const timeout = <T>(promise: Promise<T>, timeout: number) => Promise.race([
  promise,
  new Promise((resolve, reject) => {
    setTimeout(() => reject('Timed out'), timeout);
  })
])

let ready: Promise<any>;

connection.connect(function () {
  console.log('Connecting...');

  ready.then(client => {
    console.log('Ready?');
  }, (err: any) => console.error(err));
});
ready = client.setup(config);

function readyApiCall(...args: any[]) {
  return ready.then((client: any) => client.callApi(...args));
}

document.querySelector('a').addEventListener('click', function logOut() {
  readyApiCall('auth.logOut').then((d: any) => console.log(d));
});

// Telegram public keys
function addPublicKeys(telegram: Telegram) {
  let publicKeys = [{
    fingerprint: '0x9a996a1db11c729b',
    modulus: '',
    exponent: '010001'
  },
    {
      fingerprint: '0xb05b2a6f70cdea78',
      key: '',
      exponent: '010001'
    },
    {
      fingerprint: '0xc3b42b026ce86b21',
      modulus: '',
      exponent: '010001'
    },
    {
      fingerprint: '0x71e025b6c76033e3',
      modulus: '',
      exponent: '010001'
    }];

  publicKeys.forEach(key => telegram.addPublicKey(key));
}


